class Evts {
  ...
  addEvent(eventName, element, callback, scope = element, options = false) {
    // Debug logging
    this._raise('log', `Evts.addEvent: ${eventName} ${element} ${callback} ${scope} ${options}`);
    ...
    const err = () => {
      this._raise('error', 'Evts.addEvent: Wrong type for argument');
    };
  ...
  _clearRegularEvent(index) {
    // Debug logging
    this._raise('log', `Evts._clearRegularEvent: ${index}`);
    // Missing mandatory arguments
  ...
  subscribe(eventName, callback, oneShot = false) {
    // Debug logging
    this._raise('log', `Evts.subscribe: ${eventName} ${callback} ${oneShot}`);
    // Missing mandatory arguments
    if (eventName === null || eventName === undefined ||
      callback === null || callback === undefined) {
    ...
    // Prevent wrong type for arguments (mandatory and optional)
    const err = () => {
      this._raise('error', 'Evts.subscribe: Wrong type for argument');
    };
  ...
  unsubscribe(eventId) {
    // Debug logging
    this._raise('log', `Evts.unsubscribe: ${eventId}`);
    // Missing mandatory arguments
    if (eventId === null || eventId === undefined) {
  ...
  unsubscribeAllFor(eventName) {
    // Debug logging
    this._raise('log', `Evts.unsubscribeAllFor: ${eventName}`);
    // Missing mandatory arguments
    if (eventName === null || eventName === undefined) {
  ...
  publish(eventName, data = null) {
    // Debug logging
    this._raise('log', `Evts.publish: ${eventName} ${data}`);
    // Missing mandatory arguments
    if (eventName === null || eventName === undefined) {
    ...
    for (let i = 0; i < keys.length; ++i) {
      ...
      if (keys[i] === eventName) {
        ...
        for (let j = (subs.length - 1); j >= 0; --j) {
          // Debug logging
          this._raise('log', `Evts.publish: fire callback for ${eventName}, subscription n°${subs[j].id}`, subs[j]);
          // Fire saved callback
